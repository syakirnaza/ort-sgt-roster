...
def generate_medical_roster(month_idx, year, staff_df, leave_df, config_df):
    ...
    # Config columns: 1=PH, 2=ELOT, 3=Minor
    ...
    for day in days:
        ...
        # --- MAPPING UPDATED ---
        # Column D (Index 3) = Leave (Absent)
        # Column E (Index 4) = No Oncall
        ...
        if is_special:
            ...
            ae, am = get_avail(pool_elot), get_avail(pool_minor)
...
if staff is not None:
    ...
    if st.button("Generate Roster"):
        ...
        # Display Table
        st.subheader(f"Schedule: {m_name} 2026")
        st.dataframe(df_final, use_container_width=True)
        
        # WORKLOAD SUMMARY TABLE
        st.subheader("📊 Workload Fairness Summary")
        all_assigned = pd.concat([df_final['Oncall_1'], df_final['Oncall_2'], df_final['Oncall_3']])
        all_assigned = all_assigned[all_assigned != ""].value_counts().reset_index()
        all_assigned.columns = ['Staff Name', 'Total Oncall Shifts']
        st.table(all_assigned)
